...
function ProjectCard({ project, onClick, isSelected }) {
  return (
    <motion.div 
      layout
      className={`bg-black text-white p-2 lg:h-[800px] flex flex-col justify-between shadow-lg rounded-lg overflow-hidden ${isSelected ? 'fixed inset-4 z-50 flex items-center justify-center' : ''}`}
      ...
      {!isSelected ? (
        <>
          ...
          <motion.div className="p-4 lg:p-6 flex flex-col justify-between h-1/2">
            <div>
              <motion.h2 layoutId={`project-title-${project.id}`} className="text-xl lg:text-2xl font-semibold mb-2">{project.title}</motion.h2>
              <motion.p layoutId={`project-description-${project.id}`} className="text-sm lg:text-base text-white mb-2 lg:mb-4">{project.description}</motion.p>
              <motion.div layoutId={`project-tags-${project.id}`} className="flex flex-wrap gap-1 lg:gap-2 mb-2 lg:mb-4">
